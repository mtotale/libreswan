...
/* add bypass policies/holes for udp, tcp and icmp */
...
static bool netlink_port_bypass()
{
	bool ok = netlink_bypass_policy(AF_INET, IPPROTO_UDP, pluto_port);
	ok &= netlink_bypass_policy(AF_INET6, IPPROTO_UDP, pluto_port);
	ok &= netlink_bypass_policy(AF_INET, IPPROTO_UDP, pluto_nat_port);
	ok &= netlink_bypass_policy(AF_INET6, IPPROTO_ICMPV6, ICMP_NEIGHBOR_DISCOVERY);
	ok &= netlink_bypass_policy(AF_INET6, IPPROTO_ICMPV6, ICMP_NEIGHBOR_SOLICITATION);
	if (pluto_tcpport != 0) {
		ok &= netlink_bypass_policy(AF_INET, IPPROTO_TCP, pluto_tcpport);
	}
...
const struct kernel_ops netkey_kernel_ops = {
	...
	.port_bypass = netlink_port_bypass,
};
